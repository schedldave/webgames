...
class SetUniformSGNode extends SGNode {
  ...
  setUniforms(context,uniforms) {
    ...
    Object.keys(uniforms).forEach(function(key) {
      ...
      if(loc)
      {
        const prevValue = gl.getUniform(shader, loc);
        that.prevValues[key] = prevValue;
  ...
  render(context) {
    if (context.shader) {
      this.prevValues = {};
      this.setUniforms(context,this.uniforms);
    }
    //render children
    super.render(context);

    // restore previous uniforms
    if (context.shader) {
      this.setUniforms(context,this.prevValues);
    }
  }
